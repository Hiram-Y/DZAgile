...
public abstract class DCActivity extends FragmentActivity {
	...
	private DCiTopView _topViewLoader;
	private DCiBottomView _bottomViewLoader;
	...
	protected final void registerDispenseData() {
		...
		intentFilter.addAction(DCBroadcaseDefine.BROADCASE_DISPENSE_MESSAGE);
	...
	public class DCDispenseDataBroadcast extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (where == DCSendDataDefine.SEND_TO_VIEW) {
				dispenseData(object, code);
			} else if (where == DCSendDataDefine.SEND_TO_ACTIVITY) {
				onReceiveData(object, code);
			} else if (where == DCSendDataDefine.SEND_TO_ALL) {
				onReceiveData(object, code);
				dispenseData(object, code);
			}
		}
	...
	protected final void sendExitBroadcast() {
		...
		intent.setAction(DCBroadcaseDefine.EXIT_APP);
		sendBroadcast(intent);
	}

	public void setTopView(DCiTopView topViewLoader) {
		_topViewLoader = topViewLoader;
	}

	public void setBottomView(DCiBottomView bottomViewLoader) {
	...
	protected abstract DCiBottomView getBottomViewLoader();
