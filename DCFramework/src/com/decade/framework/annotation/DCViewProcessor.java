...
public class DCViewProcessor {
	...
	private static void bindViews(Activity activity, Class<?> cl) {
		try {
			for (Field field : cl.getDeclaredFields()) {
				DCInjectView av = field.getAnnotation(DCInjectView.class);
				if (av != null) {
			...
			DCLog.w(DCViewProcessor.class, e);
		}
	}
		...
		try {
			for (Field field : cl.getDeclaredFields()) {
				...
				if (av != null) {
					field.setAccessible(true);
					if (av.id() == -1) {
						int resId = ((DCBaseView) object).getIdentifier(
								field.getName(), "id",
								((DCBaseView) object).getPackageName());
	...
	private static void bindMethods(View rootView, Object object) {
		...
		for (Method method : cl.getDeclaredMethods()) {
			DCInjectClick oc = method.getAnnotation(DCInjectClick.class);
			if (oc != null) {
				DCInjectEventListener listener = new DCInjectEventListener(
	...
	private static void setEventListener(Object object, Field field,
			DCInjectView av) throws IllegalArgumentException,
			IllegalAccessException {
		Object obj = field.get(object);
		DCInjectEventListener l = new DCInjectEventListener(object);
		...
		if (obj instanceof AdapterView<?>) {
			...
			DCInjectItemSelect itemSelect = av.onItemSelect();
