...
public abstract class DCBaseViewActivity extends DCActivity {
	...
	// view堆栈
	private SparseArray<DCBaseView> _allViews;
	private List<Integer> _viewsStack;
	private DCiViewFactory _viewFactory;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		_allViews = new SparseArray<DCBaseView>();
	}
	...
	final void setViewInto(DCBaseView view, Object data, boolean cacheOpen,
	...
	// 获取一个VIEW
	final DCBaseView getView(int viewId) {
		DCBaseView view;
	...
	private final DCBaseView getViewById(int viewId) {
		DCBaseView view;
		if (_allViews.indexOfKey(viewId) >= 0) {
			view = _allViews.get(viewId);
		} else {
	...
	private DCBaseView createViewFromFactory(int viewId) {

		if (_viewFactory != null) {
			return _viewFactory.createView(this, viewId, getRootView());
		} else {
			DCLog.e(getClass(), "View factory not initialize !");
		}
		return null;
	}
	...
	protected final void preViewLast(boolean animation) {
		...
		if (viewId != -1) {
			DCBaseView view = getViewById(viewId);
	...
	public void startPage(int viewId, Object data, boolean openCache) {
		DCBaseView view = getView(viewId);
	...
	protected final void dispenseData(Object args, int code) {
		if (_allViews != null && _curryView != null) {
			if (_curryView.doingReceiveDataListener(args, code)) {
				...
				for (int i = 0; i < size; i++) {
					DCBaseView item = _allViews.valueAt(i);
	...
	private final void closeAllView() {
		if (_allViews != null) {
			int size = _allViews.size();
			for (int i = 0; i < size; i++) {
				DCBaseView view = _allViews.valueAt(i);
				view.pause();
				view.close();
				view.destroy();
			}
	...
	protected final DCBaseView getCurrentView() {
		return _curryView;
	}

	protected void setViewFactory(DCiViewFactory viewFactory) {
