...
import com.decade.agile.DCInstallApkActivity;
import com.decade.agile.R;
import com.decade.agile.components.DCRectToast;
import com.decade.agile.components.DCRectToast.ToastTheme;
import com.decade.agile.kit.DCDownloadFileAsyncTask.DCDownloadCallback;
import com.decade.framework.DCApplication;

public class DCInstallApk implements DCDownloadCallback {
	private NotificationManager nm;
	private Context _context;
	private int noteId;
	private String _appName;

	public DCInstallApk(String appName, String url) {
		_context = DCApplication.getAppContext();
		_appName = appName;
	...
	public void onDownloadEnd(int resultCode, File file) {
		if (resultCode == DCDownloadFileAsyncTask.DONE) {
			finishDownLoadNotification();
			installApk(file);
		} else if (resultCode == DCDownloadFileAsyncTask.ERROR) {
			nm.cancel(noteId);
			DCRectToast.showToastLong(_context, "下载失败", ToastTheme.ERROR);
		}
	}
	...
	private void finishDownLoadNotification() {
		Intent intent = new Intent(_context, DCInstallApkActivity.class);
