...
public class DLClassLoader extends DexClassLoader {
    ...
    private static final HashMap<String, DLClassLoader> pluginLoader = new HashMap<String, DLClassLoader>();
     ...
     * 返回dexPath对应的加载器
     */
    public static DLClassLoader getClassLoader(String dexPath, Context cxt,
            ClassLoader parent) {
        DLClassLoader cjLoader = pluginLoader.get(dexPath);
        if (cjLoader == null) {
            // 获取到app的启动路径
            final String dexOutputPath = cxt
                    .getDir("dex", Context.MODE_PRIVATE).getAbsolutePath();
            cjLoader = new DLClassLoader(dexPath, dexOutputPath, null, parent);
