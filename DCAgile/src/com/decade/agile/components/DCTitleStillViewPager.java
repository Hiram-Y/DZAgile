 ...
 * 
 */
public class DCTitleStillViewPager extends LinearLayout implements
		OnClickListener {
	...
	public DCSyncHorizontalScrollView mHorizontalScrollView;// 上面的水平滚动控件
	...
	public DCTitleStillViewPager(Context context) {
		super(context);
		mContext = context;
	}

	public DCTitleStillViewPager(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		screenWidth = DCApplication.getApp().getWorkSpaceWidth();// 获取分辨率宽度
		initRootView();
	}
	...
	private void initRootView() {
		...
		progressBar.setProgressDrawable(mContext.getResources().getDrawable(
				progressDrawable));
		mHorizontalScrollView = (DCSyncHorizontalScrollView) mrootLayout
