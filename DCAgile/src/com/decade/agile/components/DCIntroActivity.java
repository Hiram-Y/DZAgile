...
import com.decade.agile.kit.DCHandlerFactory;
...
public abstract class DCIntroActivity extends Activity {
	...
	private void startOnBackground() {
		DCHandlerFactory.newBackgroundHandler("intro_bg").post(new Runnable() {
	...
	protected static class UIHandler extends Handler {
		...
		private WeakReference<DCIntroActivity> activity;

		public UIHandler(DCIntroActivity activity) {
			this.activity = new WeakReference<DCIntroActivity>(activity);
		}
		...
		public void handleMessage(android.os.Message msg) {
			if (msg.what == INTRO_PLAY) {
				...
				DCIntroActivity intro = activity.get();
				if (intro != null) {
